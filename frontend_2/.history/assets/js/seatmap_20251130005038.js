...
async function createBooking(showtimeId, seatsPayload) {
  ...
  async function sendSeatHold(seatItems, action) {
    ...
    if (!id || !seatItems || !seatItems.length) return;

    console.log("sendSeatHold:", JSON.stringify(seatItems));
console.log("normalizedSeats:", JSON.stringify(normalizedSeats));
    const now = Date.now();
    ...
    lastHoldCall = now;
     ...
     const normalizedSeats = seatItems.map(item => {
    if (typeof item === "string") {
      return { code: item, type: "adult" };
    }
    return {
      code: item.code,
