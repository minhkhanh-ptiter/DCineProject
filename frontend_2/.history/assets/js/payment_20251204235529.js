      ...
      try {
        ...
        if (res.ok) {
          ...
          if (order) {
              ...
              // GIẢI PHÁP: GIỮ BOOKING ID CŨ
              if (state.order?.bookingId) {
                  order.bookingId = state.order.bookingId;
              }
  ...
  async function applyVoucher(code) {
    ...
    fetch(API + '/checkout/apply-voucher', {
      ...
      credentials: 'include'
        ...
        const order = parseOrder(data);
        if (order) {
            const oldBookingId = state.order?.bookingId; 
          if (oldBookingId) {
              order.bookingId = oldBookingId;
          }
