...
async function createBooking(showtimeId, seatsPayload) {
 ...
 async function sendSeatHold(items, action) {
  ...
  // Nếu không còn seat nào → release-all
  if (!items || items.length === 0) {
    console.log("sendSeatHold: release-all");
    return await apiPost(`/showtimes/${encodeURIComponent(id)}/holds`, {
      seats: [],
      action: "release"
    });
  }
  ...
  // ⭐ Chuẩn hoá payload BE yêu cầu
  const seatList = items.map(s => {
    if (typeof s === "string") {
      return { 
        code: s, 
        type: state.assigned.get(s) || "adult" 
      };
    }
    return {
      code: s.code,
  ...
  console.log("sendSeatHold payload:", seatList);
