...
public class BookingService {
    ...
    public BookingResponse createBooking(Long accountId, Long showtimeId, List<BookingRequest.SeatRequest> requestedSeats){
        ...
        for(BookingRequest.SeatRequest reqSeat : requestedSeats){
            ...
            Long price = caculatePrice(seatTypeRepo, basePrice, zone, type);
            total += price;
            items.add(new BookingResponse.Item(reqSeat.getCode(), zone ,type, price));
            
        }
        Booking booking = new Booking();
        booking.setAccountId(0L);
